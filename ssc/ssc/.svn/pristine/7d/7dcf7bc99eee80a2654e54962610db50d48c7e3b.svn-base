...
class Sxkl10fController  extends CommonController {
    /**
     * lotteryid:78&flag:mjwf
     */
    public function return_sxkl10f_mj() {
     ...
     * 山西快乐10分  返回值给前端
     */
    public function sxkl10f_openData() {
        $data['cate_id'] = 78; 
        $data['timeStamp'] = array('EGT',  time()); 
        ...
        foreach ($list as $k => $v) {
            if($k == 0){
                ...
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>78,'expect'=>$lastIssue['period']))->find();
        ...
        $pri_issues = $_pri_lastopen.($this->issue_cqssc(78));
     ...
     * 山西快乐10分  接收值、处理
     */
    public function sxkl10f_return() {
        $chase = array();//追号数据
        if($_POST['lt_project']){
            //判断投注期数是否截至
