...
class Jx11x5Controller  extends CommonController {
    
    /**
     * lotteryid:7&flag:mjwf
     */
    public function return_jx11x5_mj() {
     ...
     * 江西11选5  返回值给前端
     */
    public function jx11x5_openData() {
        $data['cate_id'] = 7; 
        $data['timeStamp'] = array('EGT',  time()); 
        ...
        foreach ($list as $k => $v) {
            if($k == 0){
                ...
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>7,'expect'=>$lastIssue['issue']))->find();
        ...
        $pri_issues = $_pri_lastopen.($this->issue_cqssc(32));
