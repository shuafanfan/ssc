...
class Tianjinkl10fController  extends CommonController {
    /**
     * lotteryid:18&flag:mjwf
     */
    public function return_tianjinkl10f_mj() {
     ...
     * 天津快乐10分  返回值给前端
     */
    public function tianjinkl10f_openData() {
        $data['cate_id'] = 18; 
        $data['timeStamp'] = array('EGT',  time()); 
        ...
        foreach ($list as $k => $v) {
            if($k == 0){
                ...
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>18,'expect'=>$lastIssue['period']))->find();
        ...
        $pri_issues = $_pri_lastopen.($this->issue_cqssc(18));
     ...
     * 天津快乐10分  接收值、处理
     */
    public function tianjinkl10f_return() {
        $chase = array();//追号数据
        if($_POST['lt_project']){
            //判断投注期数是否截至
...
//                print_r(M('cate_date_time')->getLastSql());exit;
    ...
    public function cqssc_status() {
        ...
        $count = substr($cate_date_time_info['issue'],8,2);  
        $left = 84  - $count;
