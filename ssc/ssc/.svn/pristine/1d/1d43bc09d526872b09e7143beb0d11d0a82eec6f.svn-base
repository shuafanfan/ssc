...
class Bjpk10DataController  extends CommonController {
    
    /**
     * lotteryid:26&flag:mjwf
     */
    public function return_bjpk10_mj() {
     ...
     * 北京PK拾  返回值给前端
     */
    public function bjpk10_openData() {
        $data['cate_id'] = 26; 
        $data['timeStamp'] = array('EGT',  time()); 
...
//        print_r($list);
        ...
        foreach ($list as $k => $v) {
            if($k == 0){
                ...
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>26,'expect'=>$lastIssue['period']))->find();
        ...
        $pri_issues = $_pri_lastopen.($this->issue_cqssc(26));
    ...
    public function cqssc_status() {
        ...
        $condition['date'] = date('Y-m-d');
        $condition['issue'] = $cate_date_time_info['period'] + 1;
        $data['timeStamp'] = array('LT',time());
