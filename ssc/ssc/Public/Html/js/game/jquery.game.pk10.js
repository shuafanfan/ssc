	...
	$.fn.lt_selectarea = function (opts) {
								...
								for (j = 0; j < data_sel[1].length; j++) {
									for (k = 0; k < data_sel[2].length; k++) {
										for (L = 0; L < data_sel[3].length; L++) {
											if (data_sel[0][i] != data_sel[1][j] && data_sel[0][i] != data_sel[2][k] && data_sel[0][i] != data_sel[3][L] && data_sel[1][j] != data_sel[2][k] && data_sel[1][j] != data_sel[3][L] && data_sel[2][k] != data_sel[3][L]) {
												nums++
											}
										}
									}
		...
		$($.lt_id_data.id_sel_insert).unbind("click").click(function () {
			...
			if (otype == "input") {
				...
				switch (mname) {
					case "PK_JZQ4":
						error = _inputCheck_Num(11, false, _SDinputCheck, false);
						break;
					case "PK_JZQ3":
						error = _inputCheck_Num(9, false, _SDinputCheck, false);
						break;
					case "PK_JZQ2":
						error = _inputCheck_Num(5, false, _SDinputCheck, false);
						break;
					default:
						break
				}
...
//					data.code = data.code.join('').split(',');
					...
					$("#ifNewOpen").attr("src", "page/WORecord1.shtml?id=" + $.lt_lottid + "&num=5&flag=" + Math.random());
		...
		window.setTimeout(function () {
			...
			opentimerget = window.setInterval(function () {
				if ($.lt_open_status == true || _getTimes > 200) {
					if (_getTimes > 200) {
			...
			if ($.lt_time_open <= 0) {
				...
				$.each($("#showcodebox").find("span"), function (i, n) {
					if ($(this).attr("flag") == "move") {
						//$(this).html("<img src='/images/mv.gif' width='40' style='margin-top:0px;'/>");
					}
				})
		...
		if ($.lt_time_leave <= 0) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					$('#showcodebox').html('<span class="gr_s gr_sx" flag="normal">x</span><span class="gr_s gr_sx" flag="normal">x</span><span class="gr_s gr_sx" flag="normal">x</span><span class="gr_s gr_sx" flag="normal">x</span><span class="gr_s gr_sx" flag="normal">x</span><span class="gr_s gr_sx" flag="normal">x</span><span class="gr_s gr_sx" flag="normal">x</span><span class="gr_s gr_sx" flag="normal">x</span><span class="gr_s gr_sx" flag="normal">x</span><span class="gr_s gr_sx" flag="normal">x</span>');
