...
class Ah11x5Controller  extends CommonController {
    
    /**
     * lotteryid:9&flag:mjwf
     */
    public function return_ah11x5_mj() {
     ...
     * 安徽11选5 开奖记录
     */
    public function ah11x5_open_code() {
        $data['code'] = 9;
        $ah11x5_open_code = M('open_prize_data')->where($data)->order('opentimestamp DESC')->limit(10)->select();
        $return = array('ah11x5_open_code' =>$ah11x5_open_code);
     ...
     * 安徽11选5  返回值给前端
     */
    public function ah11x5_openData() {
        $data['cate_id'] = 9; 
        $data['timeStamp'] = array('EGT',  time()); 
        $list = M('cate_date_time')->where($data)->field('date,period,open_time,issue')->order('timeStamp ASC')->select();
        $pri_issues = ''; 
        $pri_cur_issue = '';
        $pri_lastopen = '';
        $_pri_lastopen = '';
        foreach ($list as $k => $v) {
            if($k == 0){
                ...
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>9,'expect'=>$lastIssue['issue']))->find();
        ...
        $pri_issues = $_pri_lastopen.($this->issue_cqssc(9));
    ...
    public function ah11x5_return() {
        ...
        if($_POST['lotteryid'] && $_POST['flag'] == 'gethistory' && $_POST['issue']){
                $expect = date("Ymd").substr($_POST['issue'],7,2); 
