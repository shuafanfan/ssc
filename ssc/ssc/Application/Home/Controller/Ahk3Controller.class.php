...
class Ahk3Controller  extends CommonController {
    
    /**
     * lotteryid:12&flag:mjwf
     */
    public function return_ahk3_mj() {
     ...
     * 安徽快3 开奖记录
     */
    public function ahk3_open_code() {
        $data['code'] = 12;
        $yunnanforever_open_code = M('open_prize_data')->where($data)->order('opentimestamp DESC')->limit(10)->select();
        $return = array('yunnanforever_open_code' =>$yunnanforever_open_code);
    ...
    public function ahk3_openData() {
        ...
        foreach ($list as $k => $v) {
            if($k == 0){
                ...
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>12,'expect'=>$lastIssue['issue']))->find();
        ...
        $pri_issues = $_pri_lastopen.($this->issue_cqssc(12));
    ...
    public function ahk3_return() {
        ...
        if($_POST['lt_project']){
            ...
            if($_POST['lt_issue_start'] && $_POST['lt_trace_if'] != 'yes'){
                ...
                $currentIssue = M('cate_date_time')->where($issue)->getField('issue');
            ...
            foreach ($_POST['lt_project'] as  $v) {
                ...
                if($_POST['lt_trace_if'] == 'yes'){
                    ...
                    foreach ($_POST['lt_trace_issues'] as $value) {
                        ...
                        $conn=mysql_connect("118.89.29.73","root","root123") or die('连接错误：'.mysql_error());
                        if($conn)
                        {  
        ...
        if($_POST['lotteryid'] && $_POST['flag'] == 'gethistory' && $_POST['issue']){
                $expect = $_POST['issue'];
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>$_POST['lotteryid'],'expect'=>'20'.$expect))->find();
    ...
    public function cqssc_status() {
        ...
        $condition['issue'] = date('Ymd').$count;
        $data['timeStamp'] = array('LT',time());
