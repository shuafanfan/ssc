...
class Hljkl10fController  extends CommonController {
    
    /**
     * lotteryid:75&flag:mjwf
     */
    public function return_hljkl10f_mj() {
     ...
     * 黑龙江快乐10分  返回值给前端
     */
    public function hljkl10f_openData() {
        $data['cate_id'] = 75; 
        $data['timeStamp'] = array('EGT',  time()); 
        ...
        foreach ($list as $k => $v) {
            if($k == 0){
                ...
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>75,'expect'=>$lastIssue['period']))->find();
        ...
        $pri_issues = $_pri_lastopen.($this->issue_cqssc(75));
     ...
     * 黑龙江快乐10分  接收值、处理
     */
    public function hljkl10f_return() {
        $chase = array();//追号数据
        if($_POST['lt_project']){
            //判断投注期数是否截至
    ...
    public function cqssc_status() {
        ...
        $cate_date_time_info = M('cate_date_time')->where($data)->order('period DESC')->find();
        ...
        $count = substr($cate_date_time_info['issue'],5,2);  
        $left = 84  - $count;
