...
class ShsslController  extends CommonController {
    
    /**
     * lotteryid:26&flag:mjwf
     */
    public function return_shssl_mj() {
     ...
     * 上海时时乐  返回值给前端
     */
    public function shssl_openData() {
        $data['cate_id'] = 83; 
        $data['timeStamp'] = array('EGT',  time()); 
        ...
        foreach ($list as $k => $v) {
            if($k == 0){
                ...
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>83,'expect'=>$lastIssue['issue']))->find();
        ...
        $pri_issues = $_pri_lastopen.($this->issue_cqssc(83));
     ...
     * 上海时时乐  接收值、处理
     */
    public function shssl_rquest_return() {
        $chase = array();//追号数据
        if($_POST['lt_project']){
            //判断投注期数是否截至
