...
class Gxk3Controller  extends CommonController {
    /**
     * lotteryid:15&flag:mjwf
     */
    public function return_gxk3_mj() {
...
//            print_r($return);exit;
    ...
    public function gxk3_openData() {
        ...
        foreach ($list as $k => $v) {
            if($k == 0){
                ...
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>15,'expect'=>$lastIssue['issue']))->find();
        ...
        $pri_issues = $_pri_lastopen.($this->issue_cqssc(15));
    ...
    public function gxk3_return() {
        ...
        if($_POST['lt_project']){
            ...
            if($_POST['lt_issue_start'] && $_POST['lt_trace_if'] != 'yes'){
                ...
                $currentIssue = M('cate_date_time')->where($issue)->order('issue DESC ')->getField('period');
    ...
    public function cqssc_status() {
        ...
        $cate_date_time_info = M('cate_date_time')->where($data)->order('open_time DESC')->find();
        ...
        $count = substr($cate_date_time_info['issue'],9,2);  
        $left = 78  - $count;
