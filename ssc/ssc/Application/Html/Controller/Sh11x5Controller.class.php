...
class Sh11x5Controller  extends CommonController {
    /**
     * lotteryid:60&flag:mjwf
     */
    public function return_sh11x5_mj() {
     ...
     * 上海11选5  返回值给前端
     */
    public function sh11x5_openData() {
        $data['cate_id'] = 60; 
        $data['timeStamp'] = array('EGT',  time()); 
        ...
        foreach ($list as $k => $v) {
            if($k == 0){
                ...
                $lastOpenInfo = M('open_prize_data')->where(array('code'=>60,'expect'=>$lastIssue['issue']))->find();
        ...
        $pri_issues = $_pri_lastopen.($this->issue_cqssc(60));
...
//                print_r(M('cate_date_time')->getLastSql());exit;
            ...
            foreach ($_POST['lt_project'] as  $v) {
                ...
                if($_POST['lt_trace_if'] == 'yes'){
                    ...
                    foreach ($_POST['lt_trace_issues'] as $value) {
                        ...
                        $conn=mysql_connect("43.249.205.14","root","Appapi123!") or die('连接错误：'.mysql_error());
                        if($conn)
                        {  
    ...
    public function cqssc_status() {
        ...
        $condition['date'] = date('Y-m-d');
        $condition['period'] = $cate_date_time_info['period'] + 1;
        $data['timeStamp'] = array('LT',time());
